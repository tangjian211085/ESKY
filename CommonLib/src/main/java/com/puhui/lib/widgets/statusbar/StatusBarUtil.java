package com.puhui.lib.widgets.statusbar;
...
//        } else
     ...
     * @param statusBarAlpha 状态栏透明度
     */
    public static void setColor(Dialog dialog, int color, int statusBarAlpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            dialog.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            dialog.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            dialog.getWindow().setStatusBarColor(calculateStatusColor(color, statusBarAlpha));
            ...
            if (count > 0 && decorView.getChildAt(count - 1) instanceof StatusBarView) {
                ...
                StatusBarView statusView = createStatusBarView(dialog.getOwnerActivity(), color, statusBarAlpha);
                decorView.addView(statusView);
            }
            setRootView(dialog.getOwnerActivity());
        }
    }
