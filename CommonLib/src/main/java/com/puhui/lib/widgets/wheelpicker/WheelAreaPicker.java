package com.puhui.lib.widgets.wheelpicker;
...
import com.puhui.lib.widgets.wheelpicker.model.City;
import com.puhui.lib.widgets.wheelpicker.model.Province;
...
public class WheelAreaPicker extends LinearLayout implements IWheelAreaPicker {
    ...
    private static final String SELECTED_ITEM_COLOR = "#ff0000";  //ff0000  初始值:353535
    private static final String ITEM_TEXT_COLOR = "#0000ff";  //0000ff  初始值:888888
    ...
    private void initView(Context context) {
        ...
        initWheelPicker(mWPCity, 1);
        initWheelPicker(mWPArea, 1);
    }
    ...
    private void initWheelPicker(WheelPicker wheelPicker, float weight) {
        ...
        wheelPicker.setItemTextColor(Color.parseColor(ITEM_TEXT_COLOR));
        wheelPicker.setCurved(true);
        mLayoutParams.width = getContext().getResources().getDisplayMetrics().widthPixels / 3;
        wheelPicker.setLayoutParams(mLayoutParams);
        addView(wheelPicker);
    }

    public void setProvinceData(List<Province> provinceList) {
        if(null == provinceList || provinceList.size() == 0){
            throw new IllegalArgumentException("the size of provinceList is must not null or zero");
        }
    ...
    private void addListenerToWheelPicker() {
        ...
        mWPProvince.setOnItemSelectedListener(new WheelPicker.OnItemSelectedListener() {
            ...
            public void onItemSelected(WheelPicker picker, Object data, int position) {
                ...
                if(null != listener){
                    listener.onSelectedItemChanged(mWPProvince.getCurrentItemPosition(),
                            mWPCity.getCurrentItemPosition(), mWPArea.getCurrentItemPosition());
                }
            }
        });
    ...
    public void setOnSelectedItemChangeListener(OnSelectedItemChangeListener onSelectedItemChangeListener){
        listener = onSelectedItemChangeListener;
    }

    public interface OnSelectedItemChangeListener{
        void onSelectedItemChanged(int provincePosition, int cityPosition, int areaPosition);
    }
